...
yt.setKey(process.env.YOUTUBE_API);
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${process.env.PREFIX}Music`, {
    type: "STREAMING",
...
function musicFunction() {
  ...
  const dcEmbed = new MessageEmbed()
    ...
    .setDescription(
      "This command is still in testing phase. There may be an error."
    )
    .setThumbnail("https://c.tenor.com/bOR-CXcBQ8QAAAAC/djaymano-dj.gif")
    .addFields(
      {
        name: `${process.env.PREFIX}play [Name or Link]`,
        value: "This command is used to request songs.",
        inline: false,
      },
      {
        name: `${process.env.PREFIX}skip`,
        value: "We will skip songs with this command.",
        inline: false,
      },
      {
        name: `${process.env.PREFIX}stop`,
        value: "If you want to stop the music.",
        inline: false,
      },
      { name: `${process.env.PREFIX}list`, value: "Music List", inline: false },
      {
        name: `${process.env.PREFIX}loop`,
        value: "If you want to loop the music.",
        inline: false,
      }
    )
    .setTimestamp()
...
function ListEmbed(barColor, title, des, img, inVoiceCN, loop) {
  if (inVoiceCN === true) {
    ...
    const dcEmbed = new MessageEmbed()
      ...
      .setDescription(des)
      .setThumbnail(img)
      .addFields({ name: "Loop", value: check, inline: false })
      .setTimestamp()
      .setFooter(client.user.username, client.user.avatarURL());
    return dcEmbed;
  } else {
...
async function execute(msg, serverQueue) {
  ...
  if (msg.content.includes("https://") !== true) {
      ...
      if (error) {
        msg.channel.send(
          embed(
            "#FF6767",
            "Music | Error 📻",
            "I didn't find the song you requested \nplease check the correctness!",
            "https://media3.giphy.com/media/TqiwHbFBaZ4ti/giphy.gif",
            true
          )
        );
        console.log(error);
    ...
    msg.content.includes("&list")
  ) {
    try {
      var mixPlayListURL = msg.content.slice(
        "https://www.youtube.com/watch?v=".length + 6
      );
      ...
      setQueue("playlist").then(() => {
        ...
        msg.channel.send(
          embed(
            "#CF90E5",
            "Music | Playlist 📻",
            `${mixPlaylist.title} has been added to the queue!`,
            song.image,
            true
          )
        );
    ...
    if (!voiceChannel)
      return msg.channel.send(
        embed(
          ...
          "https://c.tenor.com/iNu8LXx2ECgAAAAC/senko-poute-hmph.gif",
          true
        )
      );
    ...
    if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) {
      return msg.channel.send(
        embed(
          ...
          "https://c.tenor.com/Q0HUwg81A_0AAAAC/anime-cry.gif",
          true
        )
      );
    }
  ...
  if (!msg.member.voice.channel)
    return msg.channel.send(
      embed(
        "#FF2727",
        "Music | Skip 📻",
        `You're skipping the song, but you're not listening!!`,
  ...
  if (!serverQueue)
    return msg.channel.send(
      embed(
        "#CF90E5",
        "Music | Skip 📻",
        `Hmm.. seems like there's no song to skip!?`,
  ...
  if (!msg.member.voice.channel)
    return msg.channel.send(
      embed(
        "#FF2727",
        "Music |  Disconnect 📻",
        `Ha!! You're turning off the music, but you're not listening!`,
        "https://c.tenor.com/X3x3Y2mp2W8AAAAM/anime-angry.gif",
        true
      )
    );
  ...
  if (!serverQueue)
    return msg.channel.send(
      embed(
        "#FF2727",
        "Music |  Disconnect 📻",
        `Hmm.. seems like there's no song to turn off!?`,
        "https://cdn35.picsart.com/141906934001202.gif?to=min&r=640",
        true
      )
    );
    ...
    try {
      ...
      return msg.channel.send(
        embed(
          ...
          "https://gifimage.net/wp-content/uploads/2017/11/gif-kawaii-anime-10.gif",
          true
        )
      );
    }
...
function play(guild, song) {
  ...
  if (!song) {
      ...
      if (stopByUser === false) {
        serverQueue.textChannel.send(
          embed(
            "#FF2C2C",
            "Music | Disconnect 📻",
            "I already sang all the songs you requested. Let's me rest for a while",
            "https://c.tenor.com/Ftfa-ehSIs4AAAAM/miyako-hoshino-wataten.gif",
            true
          )
        );
      }
    ...
    serverQueue.textChannel.send(
      embed(
        "#FF2C2C",
        "Music | Skip 📻",
        "There's no next song, right? \nIf you don't ask for the song, I'll be gone.",
        "https://i.pinimg.com/originals/d5/a2/b0/d5a2b01b8294bfb8678d67342b106795.gif",
        true
      )
    );
    return;
  ...
  serverQueue.textChannel.send(
    embed(
      "#CF90E5",
      "Music | Play 📻",
      `i will sing: **${song.title}** for you, hehe.`,
      "https://thumbs.gfycat.com/UnsungRespectfulBergerpicard-max-1mb.gif",
      true
    )
  );
  ...
  if (!serverQueue) {
    msg.channel.send(
      ListEmbed(
        ...
        "https://cdna.artstation.com/p/assets/images/images/038/072/082/original/perrine-huguenot-radio.gif?1622088740&dl=1",
        false,
        loop
      )
    );
  ...
  if (!msg.member.voice.channel) {
    msg.channel.send(
      embed(
        "#CF90E5",
        "Music | Skip 📻",
        `Haha! You will loop the music Even though I myself didn't listen!?`,
        "https://c.tenor.com/X3x3Y2mp2W8AAAAM/anime-angry.gif",
        true
      )
    );
    ...
    msg.channel.send(
      embed(
        "#FF7388",
        "Music | Loop",
        "Hmm.. seems like there is no song to loop.",
        "https://cdna.artstation.com/p/assets/images/images/038/072/082/original/perrine-huguenot-radio.gif?1622088740&dl=1",
        true
      )
    );
